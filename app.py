...
def scrape_pdf(url, company, year, quarter, click):
    ...
    if 'btn-scrape' in changed_id:
        ...
        return {"display":"inline"}, {"display":"inline"}, {"display":"block"}, new_df.to_dict('records'), [{"name": i, "id": i} for i in new_df.columns], new_json
...
def update_global(approve, reject, company, year, quarter,json_store):
    changed_id = [p['prop_id'] for p in callback_context.triggered][0]
